...
IF EXISTS( SELECT * FROM sysdatabases where(name='BDVichayal')) 
	begin
		DROP DATABASE BDVichayal
	end
GO

CREATE DATABASE BDVichayal
GO
...
CREATE TABLE PROVINCIA(
  ...
  PROV_DEPARTAMENTO int references DEPARTAMENTO,
...
CREATE TABLE DISTRITO(
  ...
  DIST_PROVINCIA INT references PROVINCIA,
  CONSTRAINT PK_distrito_id PRIMARY KEY (DISTRITO_ID)
  )

CREATE TABLE NIVEL(
  NIVEL_ID INT IDENTITY(1,1) NOT NULL,
  NIV_DESCRIPCION VARCHAR(45) NOT NULL,
  CONSTRAINT PK_nivel_id PRIMARY KEY (NIVEL_ID)
  )

CREATE TABLE GRADO(
  GRADO_ID INT IDENTITY(1,1) NOT NULL,
  GRA_DESCRIPCION VARCHAR(45) NOT NULL,
  CONSTRAINT PK_grado_id PRIMARY KEY (GRADO_ID)
  )

CREATE TABLE SECCION(
  SECCION_ID INT IDENTITY(1,1) ,
  SEC_DESCRIPCION VARCHAR(45) NOT NULL,
   CONSTRAINT PK_seccion_id PRIMARY KEY (SECCION_ID)
  )

CREATE TABLE ANNIO_ESCOLAR(
  ANNIO_ESCOLAR_ID INT IDENTITY(1,1),
  ANN_INICIO DATE NOT NULL,
  ANN_FIN DATE NOT NULL,
  ANN_DESCRIPCION VARCHAR(45) NOT NULL,
  CONSTRAINT PK_annio_id PRIMARY KEY (ANNIO_ESCOLAR_ID)
  )


CREATE TABLE APODERADO(
  APODERADO_ID INT IDENTITY(1,1),
  APO_DNI varchar(9) NOT NULL,
  ...
  APO_DISTRITO INT REFERENCES DISTRITO NOT NULL,
  APO_DIRECCION VARCHAR(45) NOT NULL,
   CONSTRAINT PK_apoderado_id PRIMARY KEY (APODERADO_ID)
  )

CREATE TABLE USUARIO(
  USUARIO_ID INT IDENTITY(1,1),
  USU_USUARIO VARCHAR(9) NOT NULL,
...
CREATE TABLE ADMINISTRATIVO(
  ADMINISTRATIVO_ID INT IDENTITY(1,1),
  ADM_DNI VARCHAR(45) NOT NULL,
  ADM_NOMBRES VARCHAR(45) NOT NULL,
  ADM_APELLIDOS VARCHAR(45) NOT NULL,
  ADM_FECHA DATE NOT NULL,
  ADM_TELEFONO VARCHAR(45) NOT NULL,
  ADM_CORREO VARCHAR(45) NOT NULL,
  ADM_DISTRITO INT references DISTRITO NOT NULL,
  ADM_DIRECCION VARCHAR(45) NOT NULL,
  ADM_USUARIO INT references USUARIO NOT NULL,
  CONSTRAINT PK_administrativo_id PRIMARY KEY (ADMINISTRATIVO_ID)
  )
GO
...
CREATE TABLE ESTUDIANTE(
  ...
  EST_DISTRITO INT references DISTRITO NOT NULL,
  EST_DIRECCION VARCHAR(45) NOT NULL,
  APODERADO_ID INT references APODERADO NOT NULL,
  EST_USUARIO INT references USUARIO NOT NULL,
  CONSTRAINT PK_estudiante_id PRIMARY KEY (ESTUDIANTE_ID)
  )
GO

CREATE TABLE VACANTE(
  VACANTE_ID INT IDENTITY(1,1),
  ANN_ID INT references ANNIO_ESCOLAR NOT NULL,
  EST_ID INT references ESTUDIANTE NOT NULL,
  CONSTRAINT PK_vacante_id PRIMARY KEY (VACANTE_ID)
  )
GO

CREATE TABLE MATRICULA(
  MATRICULA_ID INT IDENTITY(1,1),
  ESTUDIANTE_ID INT references ESTUDIANTE NOT NULL,
  ANNIO_ESCOLAR_ID INT references ANNIO_ESCOLAR NOT NULL,
  NIVEL_ID INT references NIVEL NOT NULL,
  GRADO_ID INT references GRADO NOT NULL,
  SECCION_ID INT references SECCION NOT NULL,
  VAC_ID INT references VACANTE NOT NULL,
  CONSTRAINT PK_matricula_id PRIMARY KEY (MATRICULA_ID)
  )
GO

--CREACION DE TRIGGERS--

CREATE TRIGGER ESTUDIANTE_BEFORE_INSERT ON ESTUDIANTE  
FOR INSERT AS  
BEGIN
	--DECLARAR VARIABLE NUEVO DNI
	DECLARE @NEW_DNI VARCHAR(9);
	--OBTENEMOS DNI
	SELECT @NEW_DNI = EST_DNI from inserted;
	--DECLARAMOS LA VARIABLE NUEVO USUARIO
	DECLARE @NEW_USU INT;
	--VERIFICAMOS SI EXISTE USUARIO
	IF EXISTS(SELECT * FROM USUARIO WHERE USU_USUARIO = @NEW_DNI)
		BEGIN
			--OBTENEMOS EL USUARIO ANTIGUO
			SELECT @NEW_USU = USUARIO_ID from USUARIO WHERE USU_USUARIO = @NEW_DNI;
		END
	ELSE
		BEGIN
			--CREAMOS Y OBTENEMOS EL USUARIO
			INSERT INTO USUARIO(USU_USUARIO,USU_CONTRASENA) values (@NEW_DNI,@NEW_DNI);
			SELECT @NEW_USU = USUARIO_ID from USUARIO WHERE USU_USUARIO = @NEW_DNI;
		END
	--DECLARAR VARIABLE NUEVO ID
	DECLARE @NEW_ID INT;
	--OBTENEMOS EL ID DE ESTUDIANTE
	SELECT @NEW_ID = ESTUDIANTE_ID from inserted;
	--ACTUALIZAMOS LA TABLA CORRIGIENDO EL ID DE USUARIO
	UPDATE ESTUDIANTE SET EST_USUARIO=@NEW_USU WHERE ESTUDIANTE_ID=@NEW_ID;
END
GO

CREATE TRIGGER ADMINISTRATIVO_BEFORE_INSERT ON ADMINISTRATIVO  
FOR INSERT AS  
BEGIN
	--DECLARAR VARIABLE NUEVO DNI
	DECLARE @NEW_DNI VARCHAR(45);
	--OBTENEMOS DNI
	SELECT @NEW_DNI = ADM_DNI from inserted;
	--DECLARAMOS LA VARIABLE NUEVO USUARIO
	DECLARE @NEW_USU INT;
	--VERIFICAMOS SI EXISTE USUARIO
	IF EXISTS(SELECT * FROM USUARIO WHERE USU_USUARIO = @NEW_DNI)
	...
	--DECLARAR VARIABLE NUEVO ID
	DECLARE @NEW_ID INT;
	--OBTENEMOS EL ID DE ADMINISTRATIVO
	SELECT @NEW_ID = ADMINISTRATIVO_ID from inserted;
	--ACTUALIZAMOS LA TABLA CORRIGIENDO EL ID DE USUARIO
	UPDATE ADMINISTRATIVO SET ADM_USUARIO=@NEW_USU WHERE ADMINISTRATIVO_ID=@NEW_ID;
END
GO

CREATE TRIGGER ESTUDIANTE_AFTER_DELETE ON ESTUDIANTE  
AFTER  DELETE  AS  
BEGIN
	--DECLARAR VARIABLE NUEVO DNI
	DECLARE @NEW_DNI VARCHAR(45);
	--OBTENEMOS DNI
	SELECT @NEW_DNI = EST_DNI from deleted;
	--DECLARAMOS LA VARIABLE NUEVO USUARIO	
	IF EXISTS(SELECT * FROM USUARIO WHERE USU_USUARIO = @NEW_DNI)
		BEGIN
			DELETE FROM USUARIO WHERE USU_USUARIO=@NEW_DNI;
		END
END
GO

CREATE TRIGGER ADMINISTRATIVO_AFTER_DELETE ON ADMINISTRATIVO  
AFTER  DELETE  AS  
BEGIN
	--DECLARAR VARIABLE NUEVO DNI
	DECLARE @NEW_DNI VARCHAR(45);
	--OBTENEMOS DNI
	SELECT @NEW_DNI = ADM_DNI from deleted;
	IF EXISTS(SELECT * FROM USUARIO WHERE USU_USUARIO = @NEW_DNI)
		BEGIN
			DELETE FROM USUARIO WHERE USU_USUARIO=@NEW_DNI;
		END
...
CREATE TRIGGER APODERADO_INSEAD_OF_DELETE ON APODERADO  
INSTEAD OF DELETE AS  
BEGIN
	--DECLARAR VARIABLE NUEVO DNI
	DECLARE @ID INT;
	--OBTENEMOS DNI
	SELECT @ID = APODERADO_ID from deleted;	
	UPDATE ESTUDIANTE SET APODERADO_ID=1 WHERE APODERADO_ID=@ID
	DELETE FROM APODERADO WHERE APODERADO_ID=@ID
END
GO

--PROCEDIMIENTOS ALMACENADOS

CREATE PROCEDURE SP_BuscarApoderado
	@texto as varchar(20)=''
as
begin
	set nocount on;
	SELECT A.APODERADO_ID,A.APO_NOMBRES, A.APO_APELLIDOS, A.APO_TELEFONO, A.APO_CORREO, A.APO_DIRECCION

	FROM APODERADO AS A
	WHERE (A.APO_DNI + '' + A.APO_NOMBRES + '' + A.APO_APELLIDOS LIKE '%' + @texto + '%')
	SET NOCOUNT OFF;
END
GO
...
begin
	set nocount on;
	select e.*,EST_DNI AS DNI,CONCAT(EST_APELLIDOS,' ',EST_NOMBRES) AS [NOMBRE COMPLETO],
	DATEDIFF ( YEAR , EST_FECHA , GETDATE()) AS EDAD,DEP.DEPA_DESCRIPCION AS DEPARTAMENTO,
	P.PROV_DESCRIPCION AS PROVINCIA,  D.DIST_DESCRIPCION AS DISTRITO, E.EST_DIRECCION AS DIRECCION,
	CONCAT(APO_APELLIDOS,' ',APO_NOMBRES) AS [APODERADO]
	from ESTUDIANTE as e
	inner join APODERADO as a on e.APODERADO_ID=a.APODERADO_ID
	inner join DISTRITO as d on e.EST_DISTRITO=d.DISTRITO_ID
	inner join PROVINCIA as p on p.PROVINCIA_ID=d.DIST_PROVINCIA
	inner join DEPARTAMENTO as DEP on p.PROV_DEPARTAMENTO=DEP.DEPARTAMENTO_ID
	WHERE (E.EST_DNI + '' + E.EST_NOMBRES + '' + E.EST_APELLIDOS LIKE '%' + @texto + '%')
	SET NOCOUNT OFF;
END
GO
...
CREATE PROCEDURE SP_DATOS_APODERADO
@texto as varchar(9)=''
as
begin
	set nocount on;
	SELECT * FROM APODERADO AS A
	WHERE A.APODERADO_ID=@texto
	SET NOCOUNT OFF;
END
GO

CREATE PROCEDURE SP_REGISTRAR_ESTUDIANTE
  @EST_DNI AS varchar(9)='',
  @EST_NOMBRES AS VARCHAR(45)='',
  @EST_APELLIDOS AS VARCHAR(45)='',
  @EST_FECHA AS DATE,
  @EST_DISTRITO AS INT,
  @EST_DIRECCION AS VARCHAR(45)='',
  @APODERADO_ID AS INT
  as
  begin
	set nocount on;

	DECLARE @iduser VARCHAR(45);

	select top 1 @iduser=USUARIO.USUARIO_ID from USUARIO 
	
	INSERT INTO ESTUDIANTE(EST_DNI,EST_NOMBRES,EST_APELLIDOS,EST_FECHA,EST_DISTRITO,EST_DIRECCION,APODERADO_ID,EST_USUARIO) VALUES (@EST_DNI,@EST_NOMBRES,@EST_APELLIDOS,@EST_FECHA,@EST_DISTRITO,@EST_DIRECCION,@APODERADO_ID,@iduser);
	SET NOCOUNT OFF;
END
GO

CREATE PROCEDURE SP_MODIFICAR_ESTUDIANTE
  @EST_DNI AS varchar(9)='',
  @EST_NOMBRES AS VARCHAR(45)='',
  @EST_APELLIDOS AS VARCHAR(45)='',
  @EST_FECHA AS DATE,
  ...
  begin
	set nocount on;
	UPDATE ESTUDIANTE
	SET EST_DNI=@EST_DNI,EST_NOMBRES=@EST_NOMBRES,EST_APELLIDOS=@EST_APELLIDOS,EST_FECHA=@EST_FECHA,EST_DISTRITO=@EST_DISTRITO,EST_DIRECCION=@EST_DIRECCION
	WHERE ESTUDIANTE_ID=@ESTUDIANTE_ID
	SET NOCOUNT OFF;
END
GO

CREATE PROCEDURE SP_DATOSDISPRODEP
@texto as INT
as
begin
	set nocount on;
	select DISTRITO_ID,DIST_DESCRIPCION,PROVINCIA_ID,PROV_DESCRIPCION,DEPARTAMENTO_ID,DEPA_DESCRIPCION
	from DISTRITO inner join PROVINCIA on PROVINCIA_ID=DIST_PROVINCIA
	inner join DEPARTAMENTO on DEPARTAMENTO_ID=PROV_DEPARTAMENTO
	where DISTRITO_ID=@texto
	SET NOCOUNT OFF;
END
GO
...
CREATE PROCEDURE SP_BuscarApoderadoGeneral
	@texto as varchar(20)=''
as
begin
	set nocount on;
	select A.*,APO_DNI AS DNI,CONCAT(APO_APELLIDOS,' ',APO_NOMBRES) AS [NOMBRE COMPLETO],
	APO_TELEFONO AS TELEFONO, APO_CORREO AS CORREO,DEP.DEPA_DESCRIPCION AS DEPARTAMENTO,
	P.PROV_DESCRIPCION AS PROVINCIA,D.DIST_DESCRIPCION AS DISTRITO, APO_DIRECCION AS DIRECCION
	from APODERADO as A
	inner join DISTRITO as d on A.APO_DISTRITO=d.DISTRITO_ID
	inner join PROVINCIA as p on p.PROVINCIA_ID=d.DIST_PROVINCIA
	inner join DEPARTAMENTO as DEP on p.PROV_DEPARTAMENTO=DEP.DEPARTAMENTO_ID
	WHERE (A.APO_DNI + '' + A.APO_NOMBRES + '' + A.APO_APELLIDOS LIKE '%' + @texto + '%') AND APODERADO_ID !=1
	SET NOCOUNT OFF;
END
GO


CREATE PROCEDURE SP_REGISTRAR_APODERADO
  @APO_DNI AS varchar(9)='',
  @APO_NOMBRES AS VARCHAR(45)='',
  @APO_APELLIDOS AS VARCHAR(45)='',
  @APO_TELEFONO AS VARCHAR(45)='',
  @APO_CORREO AS VARCHAR(45)='',
  @APO_DISTRITO AS INT,
  @APO_DIRECCION AS VARCHAR(45)=''
  as
  begin
	set nocount on;
	...
	INSERT INTO APODERADO(APO_DNI,APO_NOMBRES,APO_APELLIDOS,APO_TELEFONO,APO_CORREO,APO_DISTRITO,APO_DIRECCION) VALUES (@APO_DNI,@APO_NOMBRES,@APO_APELLIDOS,@APO_TELEFONO,@APO_CORREO,@APO_DISTRITO,@APO_DIRECCION);
	SET NOCOUNT OFF;
END
GO

CREATE PROCEDURE SP_MODIFICAR_APODERADO
  @APO_DNI AS varchar(9)='',
  @APO_NOMBRES AS VARCHAR(45)='',
  @APO_APELLIDOS AS VARCHAR(45)='',
  @APO_TELEFONO AS VARCHAR(45)='',
  ...
  begin
	set nocount on;
	UPDATE APODERADO
	SET APO_DNI=@APO_DNI,APO_NOMBRES=@APO_NOMBRES,APO_APELLIDOS=@APO_APELLIDOS,APO_TELEFONO=@APO_TELEFONO,
	APO_CORREO=@APO_CORREO,APO_DISTRITO=@APO_DISTRITO,APO_DIRECCION=@APO_DIRECCION
...
begin
	set nocount on;
	DELETE FROM APODERADO WHERE APODERADO_ID=@APO_ID
	SET NOCOUNT OFF;
END
GO
...
INSERT INTO DEPARTAMENTO (DEPARTAMENTO_ID,DEPA_DESCRIPCION) VALUES
(1, 'AMAZONAS'),
(2, 'ANCASH'),
(3, 'APURIMAC'),
(4, 'AREQUIPA'),
...
INSERT INTO PROVINCIA (PROVINCIA_ID, PROV_DESCRIPCION, PROV_DEPARTAMENTO) VALUES
(1, 'CHACHAPOYAS ', 1),
(2, 'BAGUA', 1),
(3, 'BONGARA', 1),
(4, 'CONDORCANQUI', 1),
...
INSERT INTO DISTRITO (DISTRITO_ID, DIST_DESCRIPCION, DIST_PROVINCIA) VALUES
(1, 'CHACHAPOYAS', 1),
(2, 'ASUNCION', 1),
(3, 'BALSAS', 1),
(4, 'CHETO', 1),
...
--DATO PARA FUNCIONAMIENTO--

INSERT INTO USUARIO VALUES ('00000000','00000000')
INSERT INTO APODERADO VALUES ('00000000','SIN APODERADO','SIN APODERADO','000000000','0000@0000','1','NO APLICA');
GO

--DATOS DEL APODERADO - MINIMO 50 --

	INSERT INTO APODERADO VALUES ('40262954','Henry Tomás','Carranza Morales','984632983','henrycarranza309@gmail.com','1213','Calle Lima 110 Upis Cois 3° Sector');
	INSERT INTO APODERADO VALUES ('40262955','Angela','Tarrillo Chirinos','987654456','angelacarolina@gmail.com','1','Q4CH+72X Grau, Chachapoyas');
	INSERT INTO APODERADO VALUES ('40262956','Dagner','Chuman Lluen','946061679','dagner_smooth@gmail.com','478','439H+7JJ Unión, Santa Rosa');
	INSERT INTO APODERADO VALUES ('40262957','Jose Manuel','Quiroz Quiroz','999856623','joseloe@gmail.com','1149','Miguel Grau 620, Bambamarca');
	INSERT INTO APODERADO VALUES ('40262958','Dante','Chunga Zapata','965854999','chunzap@gmail.com','1156','Casimiro Chuman N° 517');
	INSERT INTO APODERADO VALUES ('40262959','Eduado','Panta Yanovic','923912442','pantayano@gmail.com','1192','43R4+3F3, Curgos');
	INSERT INTO APODERADO VALUES ('40262960','Rayzza','Lopez Tayta','922601502','lopezrayzza@gmail.com','1203','Armas, Santa Cruz De Chuca');
	INSERT INTO APODERADO VALUES ('40262961','Lino','Parraguez Parraguez','981032014','linparra@gmail.com','1209','CG3M+MRQ, Juan Vega Ravines S/N');
	INSERT INTO APODERADO VALUES ('40262962','Fiorella','Labrin Ruiz','932732533','fiolabrrui@gmail.com','1213','64CR+276, Chiclayo');
	INSERT INTO APODERADO VALUES ('40262963','Rocio','Cespedes Tejada','921092548','tejadarocio@gmail.com','1213','Feria Cuglievan, Juan Cuglievan 856');

--DATOS DEL ESTUDIANTE - MINIMO 50 --
	--NOTA: LA FECHA SE COLOCA COMO MM/DD/YYYY
	--NOTA 2: COLOCAR EL EST_USUARIO COMO 1 EN TODOS

	INSERT INTO ESTUDIANTE VALUES ('72695477','Juan Tomas Gerald','Carranza Morales','05/08/1999','1213','Calle Lima 110 Upis Cois 3° Sector','2','1');
	INSERT INTO ESTUDIANTE VALUES ('78562359','Ricardo','Montalvan Lopez','05/08/1999','1213','64CR+276, Chiclayo','3','1');
	INSERT INTO ESTUDIANTE VALUES ('76857452','Jose Manuel','Sandoval Manayay','12/08/1999','1213','64CR+276, Chiclayo','4','1');
	INSERT INTO ESTUDIANTE VALUES ('74123551','Luis Jesus','Manayay Joe','12/09/1999','1213','64CR+276, Chiclayo','5','1');
	INSERT INTO ESTUDIANTE VALUES ('78963214','Daniel Antonio','Manrique Diaz','11/10/1999','1213','64CR+276, Chiclayo','6','1');
	INSERT INTO ESTUDIANTE VALUES ('75357589','Jesus Gamberty','Brenis Diaz','10/11/1999','1213','64CR+276, Chiclayo','7','1');
	INSERT INTO ESTUDIANTE VALUES ('79133179','Anthony','Saavedra Saavedra','05/01/1999','1213','64CR+276, Chiclayo','8','1');
	INSERT INTO ESTUDIANTE VALUES ('76498771','Beismer','Tejada Alcalde','06/02/1999','1213','64CR+276, Chiclayo','9','1');
	INSERT INTO ESTUDIANTE VALUES ('74666331','Edison','Toro Toro','09/03/1999','1213','64CR+276, Chiclayo','10','1');
	INSERT INTO ESTUDIANTE VALUES ('75847788','Dilmer','Manayay Vilcabana','10/03/1999','1213','64CR+276, Chiclayo','11','1');

--DATOS DEL ADMINISTRATIVO - MINIMO 10 --

	INSERT INTO ADMINISTRATIVO VALUES ('76333788','Roman','Monsalve Rojas','05/01/1995','958633111','roman@gmail.com','1213','José Leonardo Ortiz 566','1');
	INSERT INTO ADMINISTRATIVO VALUES ('73583999','Nilver','Juarez Millones','09/10/1994','999541248','niveljuarez@gmail.com','1213','La Victoria 244','1');
	INSERT INTO ADMINISTRATIVO VALUES ('73586974','Christian','Mendoza Siaden','07/26/1992','985565123','messicris@gmail','1213','Av. Garcilaso de la Vega 323-A, Chiclayo','1');
	INSERT INTO ADMINISTRATIVO VALUES ('73586952','Jairo Ruben','Cabrera Guevara','05/25/1990','977426111','cabrerajairo@gmail','1213','6582+982, Chiclayo','1');
	INSERT INTO ADMINISTRATIVO VALUES ('73586921','Harvin','Cercado Mundaca','12/11/1995','965325222','harvincercadom@gmail','1213','6585+J3Q, Rosales, Chiclayo','1');
	INSERT INTO ADMINISTRATIVO VALUES ('73586923','Oscar','Cumpa Veintimilla','11/21/1997','968888741','cumpaoscar@gmail','1213','Av. Chinchaysuyo 1297, La Victoria','1');
	INSERT INTO ADMINISTRATIVO VALUES ('73586152','Miguel','Montero Gutierrez','10/01/1998','963222111','chechomiguel@gmail','1213','65C8+X5V, Chiclayo','1');
	INSERT INTO ADMINISTRATIVO VALUES ('73586445','Alex','Alcàntara Monteza','05/21/1991','956235211','alexalcantara@gmail','1213','649P+7RJ Virgin Maria, Chiclayo','1');
	INSERT INTO ADMINISTRATIVO VALUES ('73588545','Celeste Kristhel','Palacios Revolledo','06/06/1991','987521124','palaceleste@gmail','1213','Nº, Prol Av Bolognesi 712, Chiclayo','1');
	INSERT INTO ADMINISTRATIVO VALUES ('73598523','Cesar','Inga Llauce','05/30/1992','965852222','cesarllauce@gmail','1213','Algarrobos 222, Chiclayo','1');
